      ...
      <nav aria-label={language === "es" ? "Navegación principal" : "Main navigation"} className={`fixed inset-x-0 top-0 z-50 transition-colors duration-300 ${isScrolled ? "border-b border-border/50 bg-background/88 backdrop-blur-xl" : "bg-transparent"}`}>
        <div className="mx-auto flex h-[4.5rem] max-w-[100rem] items-center justify-between px-4 sm:px-6 lg:px-8">
          ...
          <div className="hidden items-center gap-7 lg:flex">
            {navItems.map((item) => (
              <a key={item.href} href={item.href} aria-current={activeSection === item.href.slice(1) ? "location" : undefined} className={`relative py-2 font-mono text-[10px] font-semibold uppercase tracking-[.16em] transition-colors ${activeSection === item.href.slice(1) ? "text-primary" : "text-foreground/62 hover:text-foreground"}`}>
                {item.name}
              </a>
            ))}
          </div>
          ...
          <div className="flex items-center gap-3">
            <LanguageToggle />
            <a href="/cv/CV_Alex_Vicente_Lopez_Frontend_React_A4.pdf" download className="hidden min-h-9 items-center gap-2 border border-border/70 px-3 font-mono text-[9px] font-bold uppercase tracking-[.14em] transition-colors hover:border-primary hover:text-primary sm:inline-flex">
