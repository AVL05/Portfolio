    ...
    <main id="main-content" className="case-study min-h-screen overflow-hidden bg-background text-foreground">
      <section className="px-4 pb-16 pt-6 sm:px-6 lg:px-8">
        <div className="mx-auto max-w-7xl">
          <nav aria-label={language === "es" ? "Navegación del caso" : "Case study navigation"} className="mb-14 flex items-center justify-between border-b border-border/65 py-3">
            <Link href="/#projects" className="cinema-link font-mono text-[10px] font-bold uppercase tracking-[.15em]"><ArrowLeft />{copy.back}</Link>
            <Link href="/#contact" className="cinema-link font-mono text-[10px] font-bold uppercase tracking-[.15em]">{copy.contact}<ArrowUpRight /></Link>
          </nav>
          ...
          <div className="grid gap-10 lg:grid-cols-[minmax(0,.82fr)_minmax(380px,.68fr)] lg:items-center">
            <div>
              ...
              <div className="mt-7 border-l border-primary pl-4">
                <p className="font-mono text-[9px] uppercase tracking-[.16em] text-muted-foreground">{copy.role}</p>
      ...
      <section className="border-t border-border/55 px-4 py-16 sm:px-6 lg:px-8">
        <div className="mx-auto grid max-w-7xl gap-10 lg:grid-cols-[.75fr_1.25fr]">
          ...
          <div className="grid content-start gap-2 sm:grid-cols-3">{data.stack.map((item) => <span key={item} className="border border-border/55 px-3 py-3 font-mono text-[10px] font-bold uppercase tracking-[.1em] text-muted-foreground">{item}</span>)}</div>
        </div>
      </section>
      ...
      <section className="px-4 py-20 sm:px-6 lg:px-8">
        <div className="mx-auto grid max-w-7xl gap-10 lg:grid-cols-[.8fr_1.2fr]">
          ...
          <div><p className="mb-5 font-mono text-[10px] font-bold uppercase tracking-[.16em] text-muted-foreground">{copy.learnings}</p><TextList items={data.learnings} language={language} /><div className="mt-8 flex flex-wrap gap-4"><a href={data.github} target="_blank" rel="noopener noreferrer" className="cinema-link font-mono text-[10px] font-bold uppercase tracking-[.14em]"><FaGithub />{copy.code}</a>{data.demo ? <a href={data.demo} target="_blank" rel="noopener noreferrer" className="cinema-link font-mono text-[10px] font-bold uppercase tracking-[.14em]">{copy.demo}<ArrowUpRight /></a> : null}</div></div>
        </div>
      </section>
    </main>
  );
