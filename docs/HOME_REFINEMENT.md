...
- `components/hero.tsx`, `components/hero.module.css`: una composición amplia en lugar de tres capítulos sticky. Nombre HTML, rol Frontend / Full-Stack Developer, especialización React / Next.js, CTA a proyectos y contacto. GitHub, LinkedIn y CV quedan en una línea secundaria. Los cuatro enlaces bajo el visor reutilizan los destinos de los hotspots.
- `components/hero-3d/CreativeRoomHero.tsx`, `creative-room.module.css`, `roomSceneConfig.ts`: navegación a proyectos, fotografía, capacidades y perfil personal. Movimiento de scroll máximo de 5% vertical, escala 96% y opacidad 90%, solo en escritorio sin reducción de movimiento. Se revierte al expandir para conservar el posicionamiento y foco del diálogo.
- `components/projects.tsx`: se conservan raw.vives, LumaFlow Studio, Distrito Gourmet y Llibret Falla el Molí. Resumen breve, stack, evidencias y CTA. El archivo mantiene los otros tres proyectos en tarjetas compactas con imágenes visibles y destinos originales. Enlace discreto al GitHub real.
- `components/skills.tsx`: tres áreas, Desarrollo / Diseño e interacción / Construcción y entrega. Contenido centralizado en los locales. Se retiran la duplicación de fotografía y los párrafos repetidos de posicionamiento.
- `components/experience.tsx`: timeline compartida para empleo y formación, fechas separadas y jerarquía institución/rol. Se conservan exactamente las dos experiencias, las dos formaciones, ubicaciones, contratos y responsabilidades originales ES/EN. CTA de CV compacto.
...
- `components/currently.tsx`, `components/home-client.tsx`: Actualmente antes de contacto. Alex Creative Space / 3D · WebGL / Valencia, editable en los locales. AI Creative Assistant no está documentado en este repositorio y no se añade. Tampoco se atribuye desarrollo asistido por IA sin evidencia.
...
- `app/layout.tsx`, `lib/seo.ts`, `app/api/og/route.tsx`, `app/sobre-mi/page.tsx`: rol profesional coherente en metadata, JSON-LD, Open Graph y perfil. Ajuste del tamaño del subtítulo OG. La perspectiva personal retirada de la hero vive en Sobre mí, destino de la ventana.
